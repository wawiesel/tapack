...
MODULE USR_QuasiDiffusion02
!!## PURPOSE
!! The user-module for the second variant
...
!!## ACCESS
PUBLIC :: SETUP_QDF02,UPDATE_QDF02,RESIDUAL_QDF02,RECALL_QDF02

!!## IDENTIFICATION
CHARACTER(*),PARAMETER :: mod_="USR_QuasiDiffusion02"
...
SUBROUTINE SETUP_QDF02( Ng , Mesh , A , b , x , Jbdry , Jdomn , FdBk ,&
  Return_Interfaces )
!!#### PURPOSE
...
SUBROUTINE SETUP_QDF02_INTERIOR(g,i,Mesh,mact,macs,l_,ExtSource,&
...
SUBROUTINE PUT_QDF02(g,i,jb,jd,j)
!!#### OPTIONAL INPUT
...
SUBROUTINE RESIDUAL_QDF02( res , relres , x , iter , Mesh , &
...
DO g=1,Ng0
 ...
 DO i=1,Ni0
  ...
  CALL SETUP_QDF02_INTERIOR(g,i,Mesh,mact,macs,l_,ExtSource,&
  ...
  DO n=1,5
   IF( Unit_resid_/=0 )THEN
    CALL RESIDUAL_QDF02_INTERIOR(n,res,relres,x,label)
   ELSE
    CALL RESIDUAL_QDF02_INTERIOR(n,res,relres,x)
   END IF
  END DO
 ...
 DO jn=1,Njn0
  ...
  CALL SETUP_QDF02_INTERFACE(g,jn,Mesh,Interfaces, &
    Noisy=.FALSE.,Unit=0,Interactive=.FALSE. )

  IF( Unit_resid_/=0 )THEN
   CALL RESIDUAL_QDF02_IC(res,relres,x,label)
  ELSE
   CALL RESIDUAL_QDF02_IC(res,relres,x)
  END IF
 ...
 DO jb=1,Njb0
  ...
  CALL SETUP_QDF02_BOUNDARY(g,jb,Mesh,Jbdry,Jdomn, C , &
    ScalarFluxIN , CurrentIN , BC , LOBC , FixBdryVals , &
    Noisy=.FALSE.,Unit=0,Interactive=.FALSE.)

  !! 6.b Get the boundary conditions.
  IF( Unit_resid_/=0 )THEN
   CALL RESIDUAL_QDF02_BC(res,relres,x,label)
  ELSE
   CALL RESIDUAL_QDF02_BC(res,relres,x)
  END IF
...
IF( Unit_resid_/=0 )THEN
 WRITE(Unit_resid_,"(a,i6.3,a)")"#QDF02 RESIDUALS (iter=",iter,")#"
...
SUBROUTINE RECALL_QDF02( x , ScalarFluxC , ScalarFluxF , CurrentF )
!!#### PURPOSE
...
DO g=1,Ng0
 ...
 DO i=1,Ni0
  ...
  !cell flux
  n = COL_QDF02_Cell(QDF_Flux,QDF_Center)
  ScalarFluxC(g,i) = x(n)
 END DO
 ...
 DO j=1,Nj0
  ...
  !face flux
  n = COL_QDF02_Face(QDF_Flux)
  ScalarFluxF(g,j) = x(n)

  !face current
  n = COL_QDF02_Face(QDF_Current)
  CurrentF(g,j) = x(n)
 END DO
END DO
...
SUBROUTINE UPDATE_QDF02( A , b , Mesh , mact , macs , l_ , &
...
CALL UPDATE(fdbk_comment,fdbk,s="[[QDF]] forming the sparse coefficient &
  &matrix <A> and right side <b> for the QDF02 system variant")
...
IF( DEFAULT(.FALSE.,Noisy) )THEN
 WRITE(DEFAULT(window_unit,Unit),"(//,a)")"#QDF02 SETUP"
END IF
...
DO g=1,Ng0
 ...
 DO i=1,Ni0
  ...
  !1. for each of 5 equations
  DO n=1,5
   CALL UPDATE_QDF02_INTERIOR(n,A,b)
  END DO
 ...
 DO jn=1,Njn0
  CALL SETUP_QDF02_INTERFACE(g,jn,Mesh,Interfaces, &
    Noisy,Unit,Interactive )
  CALL UPDATE_QDF02_InterfaceCondition(A,b)
 END DO
 ...
 DO jb=1,Njb0
  ...
  !! 6.b Get the boundary conditions.
  CALL UPDATE_QDF02_BoundaryCondition(A,b)
 END DO
END DO
!(/hack/)
!write(*,*)row_used
...
CALL UPDATE(fdbk_comment,fdbk,s="[[QDF]] equation/unknown count for QDF02 system: generated <"&
...
SUBROUTINE SETUP_QDF02_INTERFACE(g,jn,Mesh,Interfaces, &
  Noisy,Unit,Interactive )
!!#### PURPOSE
...
SUBROUTINE SETUP_QDF02_BOUNDARY(g,jb,Mesh,Jbdry,Jdomn, C , &
...
SUBROUTINE UPDATE_QDF02_INTERIOR(n,A,b)
!!#### PURPOSE
...
!!--begin--
!get row
row    =   ROW_QDF02_Cell(n)
!get columns
cols   =  COLS_QDF02_Cell(n)
!get coefficients
coeff  = COEFF_QDF02_Cell(n)
...
SUBROUTINE RESIDUAL_QDF02_IC(res,relres,x,label)
!!#### PURPOSE
...
!! get row
row    =   ROW_QDF02_Interface(spec2,spec5)
!! get cols
cols   =  COLS_QDF02_Interface(spec2,spec5)
!! get coeff
coeff  = COEFF_QDF02_Interface(spec2,spec5)
!get right side
rhs    =   RHS_QDF02_Interface(spec2,spec5)
...
SUBROUTINE UPDATE_QDF02_InterfaceCondition(A,b)
!!#### PURPOSE
...
IF( InterfaceType==QDF_SC_plus_WF .OR. InterfaceType==QDF_SC_plus_WEF )THEN
 !! get row
 row    =   ROW_QDF02_Interface(QDF_Flux,QDF_Weak)
 !! get cols
 cols   =  COLS_QDF02_Interface(QDF_Flux,QDF_Weak)
 !! get coeff
 coeff  = COEFF_QDF02_Interface(QDF_Flux,QDF_Weak)
 !! get right side
 b(row) = RHS_QDF02_Interface(QDF_Flux,QDF_Weak)

!weak current
ELSE
 !! get row
 row    =   ROW_QDF02_Interface(QDF_Current,QDF_Weak)
 !! get cols
 cols   =  COLS_QDF02_Interface(QDF_Current,QDF_Weak)
 !! get coeff
 coeff  = COEFF_QDF02_Interface(QDF_Current,QDF_Weak)
 !! get right side
 b(row) = RHS_QDF02_Interface(QDF_Current,QDF_Weak)
END IF
...
DO js0=1,SIZE(Interfaces(jn0)%subs)
 ...
 IF( InterfaceType==QDF_SC_plus_WF .OR. InterfaceType==QDF_SC_plus_WEF )THEN
  !! get row
  row    =   ROW_QDF02_Interface(QDF_Current,QDF_Strong)
  !! get cols
  cols   =  COLS_QDF02_Interface(QDF_Current,QDF_Strong)
  !! get coeff
  coeff  = COEFF_QDF02_Interface(QDF_Current,QDF_Strong)
  !! get right side
  b(row) = RHS_QDF02_Interface(QDF_Current,QDF_Strong)

 !strong flux
 ELSE
  !! get row
  row    =   ROW_QDF02_Interface(QDF_Flux,QDF_Strong)
  !! get cols
  cols   =  COLS_QDF02_Interface(QDF_Flux,QDF_Strong)
  !! get coeff
  coeff  = COEFF_QDF02_Interface(QDF_Flux,QDF_Strong)
  !! get right side
  b(row) = RHS_QDF02_Interface(QDF_Flux,QDF_Strong)
 END IF
...
SUBROUTINE UPDATE_QDF02_BoundaryCondition(A,b)
!!#### PURPOSE
...
!!--begin--
!! get row
row    =   ROW_QDF02_BoundaryFace(BC0,LOBC0)
!! get cols
cols   =  COLS_QDF02_BoundaryFace(BC0,LOBC0)
!! get coeff
coeff  = COEFF_QDF02_BoundaryFace(BC0,LOBC0)
...
CALL SetRow (A, row, cols(1:N) , coeff(1:N) )
!get right side
b(row) = RHS_QDF02_BoundaryFace(BC0,LOBC0)
...
SUBROUTINE RESIDUAL_QDF02_INTERIOR(n,res,relres,x,label)
!!#### PURPOSE
...
SUBROUTINE RESIDUAL_QDF02_BC(res,relres,x,label)
!!#### PURPOSE
...
coeff  = COEFF_QDF02_BoundaryFace(BC0,LOBC0)
!get right side
rhs = RHS_QDF02_BoundaryFace(BC0,LOBC0)
...
FUNCTION RHS_QDF02_Cell(spec2) RESULT(RHS)
!!#### PURPOSE
...
FUNCTION RHS_QDF02_BoundaryFace(BC0,LOBC0) RESULT(RHS)
!!#### REQUIRED INPUT
...
FUNCTION COEFF_QDF02_BoundaryFace(BC0,LOBC0) RESULT(Coeff)
!!#### PURPOSE
...
FUNCTION COLS_QDF02_BoundaryFace(BC0,LOBC0) RESULT(COLS)
!!#### REQUIRED INPUT
INTEGER,INTENT(IN) :: BC0,LOBC0
...
SELECT CASE( LOBC0 )
 ...
 CASE(QDF_fixedJ)      ; cols(1) = COL_QDF02_Face(QDF_Current)
 CASE(QDF_fixedPhi)    ; cols(1) = COL_QDF02_Face(QDF_Flux)
 CASE(QDF_functionJ  ) ; cols(1) = COL_QDF02_Face(QDF_Current)
 CASE(QDF_functionPhi) ; cols(1) = COL_QDF02_Face(QDF_Flux)

 CASE(QDF_functionMixed) ; cols(1)=COL_QDF02_Face(QDF_Current)
                           cols(2)=COL_QDF02_Face(QDF_Flux)

 !normal handling
 CASE DEFAULT

   IF( IsReflective(BC0) )THEN
    cols(1) = COL_QDF02_Face(QDF_Current)
   ELSE
    cols(1) = COL_QDF02_Face(QDF_Current)
    cols(2) = COL_QDF02_Face(QDF_Flux   )
   END IF
...
FUNCTION COLS_QDF02_Cell( spec3 ) RESULT(cols)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_ = "COLS_QDF02_Cell"
...
SELECT CASE( spec3 )
  CASE(QDF_Moment0Center)
    cols(1) = COL_QDF02_Cell(QDF_Current,QDF_Bottom)
    cols(2) = COL_QDF02_Cell(QDF_Current,QDF_Right )
    cols(3) = COL_QDF02_Cell(QDF_Current,QDF_Top   )
    cols(4) = COL_QDF02_Cell(QDF_Current,QDF_Left  )
    cols(5) = COL_QDF02_Cell(QDF_Flux   ,QDF_Center)

  CASE(QDF_Moment1Bottom)
    cols(1) = COL_QDF02_Cell(QDF_Flux   ,QDF_Bottom)
    cols(2) = COL_QDF02_Cell(QDF_Flux   ,QDF_Right )
    cols(3) = COL_QDF02_Cell(QDF_Flux   ,QDF_Center)
    cols(4) = COL_QDF02_Cell(QDF_Flux   ,QDF_Left  )
    cols(5) = COL_QDF02_Cell(QDF_Current,QDF_Bottom)

  CASE(QDF_Moment1Right )
    cols(1) = COL_QDF02_Cell(QDF_Flux   ,QDF_Bottom)
    cols(2) = COL_QDF02_Cell(QDF_Flux   ,QDF_Right )
    cols(3) = COL_QDF02_Cell(QDF_Flux   ,QDF_Top   )
    cols(4) = COL_QDF02_Cell(QDF_Flux   ,QDF_Center)
    cols(5) = COL_QDF02_Cell(QDF_Current,QDF_Right )

  CASE(QDF_Moment1Top   )
    cols(1) = COL_QDF02_Cell(QDF_Flux   ,QDF_Center)
    cols(2) = COL_QDF02_Cell(QDF_Flux   ,QDF_Right )
    cols(3) = COL_QDF02_Cell(QDF_Flux   ,QDF_Top   )
    cols(4) = COL_QDF02_Cell(QDF_Flux   ,QDF_Left  )
    cols(5) = COL_QDF02_Cell(QDF_Current,QDF_Top   )

  CASE(QDF_Moment1Left  )
    cols(1) = COL_QDF02_Cell(QDF_Flux   ,QDF_Bottom)
    cols(2) = COL_QDF02_Cell(QDF_Flux   ,QDF_Center)
    cols(3) = COL_QDF02_Cell(QDF_Flux   ,QDF_Top   )
    cols(4) = COL_QDF02_Cell(QDF_Flux   ,QDF_Left  )
    cols(5) = COL_QDF02_Cell(QDF_Current,QDF_Left  )
...
FUNCTION COEFF_QDF02_Cell( spec3 ) RESULT(coeff)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_ = "COEFF_QDF02_Cell"
...
SUBROUTINE GetPieces(r1,r2,r3,  c1x,c2x,c3x,c1y,c2y,c3y,B)
REAL(KIND_QDF) :: r1(2),r2(2),r3(2)
REAL(KIND_QDF) :: c1x,c2x,c3x,c1y,c2y,c3y
REAL(KIND_QDF) :: r21(2),r32(2),r13(2),B
...
!special product
!from waw and maple inversion of vandermonde system to find coefficients
!of a plane that fits 3 points
!B   = r21(1)*r3(2) + r13(1)*r2(2) + r32(1)*r1(2)
!from Morel note: (x2-x1)*(y3-y1) - (y2-y1)*(x3-x1)
B = r13(1)*r21(2) - r21(1)*r13(2)
...
!write(33,*)"cx="
!write(33,*)c1x,c2x,c3x
!write(33,*)"cy="
!write(33,*)c1y,c2y,c3y
!write(33,*)"B="
!write(33,*)B
!!--end--
END SUBROUTINE
...
cvol = CellVolume  (Mesh,i)
fc0  = FaceCentroid(Mesh,j0)
fn0  = FaceNormal  (Mesh,j0)
r0   = fc0-cc
...
FUNCTION COL_QDF02_Cell( spec1 , spec2 ) RESULT(col)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_ = "COL_QDF02_Cell"
...
FUNCTION COL_QDF02_Face( spec1 ) RESULT(col)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_ = "COL_QDF02_Face"
...
FUNCTION ROW_QDF02_Cell( spec3 ) RESULT(row)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_ = "ROW_QDF02_Cell"
...
ELSE
 ...
 SELECT CASE( spec3 )
   CASE(QDF_Moment0Center) ;         ; row = COL_QDF02_Cell(QDF_Flux,QDF_Center) ! i0
   CASE(QDF_Moment1Bottom) ; j0 = jB ; row = COL_QDF02_Face(MERGE(QDF_Current,QDF_Flux,IsMaster(j0)))
   CASE(QDF_Moment1Right ) ; j0 = jR ; row = COL_QDF02_Face(MERGE(QDF_Current,QDF_Flux,IsMaster(j0)))
   CASE(QDF_Moment1Top   ) ; j0 = jT ; row = COL_QDF02_Face(MERGE(QDF_Current,QDF_Flux,IsMaster(j0)))
   CASE(QDF_Moment1Left  ) ; j0 = jL ; row = COL_QDF02_Face(MERGE(QDF_Current,QDF_Flux,IsMaster(j0)))
   CASE DEFAULT
     VS = MODPROC(mod_,proc_)//"this equation specification does not exist."
         CALL Stop(s=STR(VS))
         VS = ""
...
FUNCTION ROW_QDF02_BoundaryFace(BC0,LOBC0) RESULT(ROW)
!!#### REQUIRED INPUT
INTEGER,INTENT(IN) :: BC0,LOBC0
...
ELSE
 ...
 row = COL_QDF02_Face(QDF_Current)
 row = row + (g0-1)*offsets(3)
...
FUNCTION ROW_QDF02_InterFace(spec2,spec5) RESULT(ROW)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_="ROW_QDF02_InterFace"
...
FUNCTION COLS_QDF02_InterFace(spec2,spec5) RESULT(COLS)
!!#### REQUIRED INPUT
INTEGER,INTENT(IN) :: spec2,spec5
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_="COLS_QDF02_InterFace"
...
SELECT CASE(spec5)
  ...
  CASE(QDF_Weak)
    j0      = Interfaces(jn0)%master
    cols(1) = COL_QDF02_Face(spec2)
    DO js0 = 1,SIZE(Interfaces(jn0)%subs)
     j0          = Interfaces(jn0)%subs(js0)
     cols(1+js0) = COL_QDF02_Face(spec2)
    END DO
...
FUNCTION COEFF_QDF02_Interface(spec2,spec5) RESULT(Coeff)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_="COEFF_QDF02_InterFace"
...
FUNCTION RHS_QDF02_Interface(spec2,spec5) RESULT(rhs)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_="rhs_QDF02_InterFace"
