MODULE USR_QuasiDiffusion03
!!#### PURPOSE
!! The user-module for the third variant
!! of the quasidiffusion routines.

!!#### DETAILS
!! This variant applies a different center discretization
!! of the cell which includes path integral actually through
!! the center of any cell (02 variant did not do this).
...
!!#### ACCESS
PUBLIC :: SETUP_QDF03,UPDATE_QDF03,RESIDUAL_QDF03,RECALL_QDF03

!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: mod_="USR_QuasiDiffusion03"
...
INTEGER       :: row,cols(1:5),i0,g0,j0,jb0,jd0,jn0,js0
...
SUBROUTINE SETUP_QDF03( Ng , Mesh , A , b , x , Jbdry , Jdomn , FdBk , &
 Return_Interfaces )
!!#### PURPOSE
...
IF( ALLOCATED(lB) )THEN
 ...
 DEALLOCATE(hsubs0)
END IF
ALLOCATE(lB(NDim0),lR(NDim0),lT(NDim0),lL(NDim0))
ALLOCATE(lh(NDim0),lv(NDim0))
...
SUBROUTINE SETUP_QDF03_INTERIOR(g,i,Mesh,mact,macs,l_,ExtSource,&
...
SUBROUTINE PUT_QDF03(g,i,jb,jd,j)
!!#### OPTIONAL INPUT
...
SUBROUTINE RESIDUAL_QDF03( res , relres , x , iter , Mesh , &
...
DO g=1,Ng0
 ...
 DO i=1,Ni0
  ...
  CALL SETUP_QDF03_INTERIOR(g,i,Mesh,mact,macs,l_,ExtSource,&
  ...
  DO n=1,5
   IF( Unit_resid_/=0 )THEN
    CALL RESIDUAL_QDF03_INTERIOR(n,res,relres,x,label)
   ELSE
    CALL RESIDUAL_QDF03_INTERIOR(n,res,relres,x)
   END IF
  END DO
 ...
 DO jn=1,Njn0
  ...
  CALL SETUP_QDF03_INTERFACE(g,jn,Mesh,Interfaces, &
    Noisy=.FALSE.,Unit=0,Interactive=.FALSE. )

  IF( Unit_resid_/=0 )THEN
   CALL RESIDUAL_QDF03_IC(res,relres,x,label)
  ELSE
   CALL RESIDUAL_QDF03_IC(res,relres,x)
  END IF
 ...
 DO jb=1,Njb0
  ...
  CALL SETUP_QDF03_BOUNDARY(g,jb,Mesh,Jbdry,Jdomn, C , &
    ScalarFluxIN , CurrentIN , BC , LOBC , FixBdryVals , &
    Noisy=.FALSE.,Unit=0,Interactive=.FALSE.)

  !! 6.b Get the boundary conditions.
  IF( Unit_resid_/=0 )THEN
   CALL RESIDUAL_QDF03_BC(res,relres,x,label)
  ELSE
   CALL RESIDUAL_QDF03_BC(res,relres,x)
  END IF
...
IF( Unit_resid_/=0 )THEN
 WRITE(Unit_resid_,"(a,i6.3,a)")"#QDF03 RESIDUALS (iter=",iter,")#"
...
SUBROUTINE RECALL_QDF03( x , ScalarFluxC , ScalarFluxF , CurrentF )
!!#### PURPOSE
...
DO g=1,Ng0
 ...
 DO i=1,Ni0
  ...
  !cell flux
  n = COL_QDF03_Cell(QDF_Flux,QDF_Center)
  ScalarFluxC(g,i) = x(n)
 END DO
 ...
 DO j=1,Nj0
  ...
  !face flux
  n = COL_QDF03_Face(QDF_Flux)
  ScalarFluxF(g,j) = x(n)

  !face current
  n = COL_QDF03_Face(QDF_Current)
  CurrentF(g,j) = x(n)
 END DO
END DO
...
SUBROUTINE UPDATE_QDF03( A , b , Mesh , mact , macs , l_ , &
 ExtSource , ECxx , ECyy , ECxy , Exx , Eyy , Exy , Jbdry , Jdomn , C , &
 ScalarFluxIN , CurrentIN , BC , LOBC , FixBdryVals , &
 FdBk , Noisy,Unit,Interactive)
!!#### PURPOSE
...
CALL UPDATE(fdbk_comment,fdbk,s="[[QDF]] forming the sparse coefficient &
  &matrix <A> and right side <b> for the QDF03 system variant")
...
IF( DEFAULT(.FALSE.,Noisy) )THEN
 WRITE(DEFAULT(window_unit,Unit),"(//,a)")"#QDF03 SETUP"
END IF
...
DO g=1,Ng0
 ...
 DO i=1,Ni0
  ...
  !1. for each of 5 equations
  DO n=1,5
   CALL UPDATE_QDF03_INTERIOR(n,A,b)
  END DO
 ...
 DO jn=1,Njn0
  CALL SETUP_QDF03_INTERFACE(g,jn,Mesh,Interfaces, &
    Noisy,Unit,Interactive )
  CALL UPDATE_QDF03_InterfaceCondition(A,b)
 END DO
 ...
 DO jb=1,Njb0
  ...
  !! 6.b Get the boundary conditions.
  CALL UPDATE_QDF03_BoundaryCondition(A,b)
 END DO
END DO
!(/hack/)
!write(*,*)row_used
...
CALL UPDATE(fdbk_comment,fdbk,s="[[QDF]] equation/unknown count for QDF03 system: generated <"&
...
SUBROUTINE SETUP_QDF03_INTERFACE(g,jn,Mesh,Interfaces, &
  Noisy,Unit,Interactive )
!!#### PURPOSE
...
SUBROUTINE SETUP_QDF03_BOUNDARY(g,jb,Mesh,Jbdry,Jdomn, C , &
    ScalarFluxIN , CurrentIN , BC , LOBC , FixBdryVals , &
        Noisy,Unit,Interactive )
!!#### PURPOSE
...
SUBROUTINE UPDATE_QDF03_INTERIOR(n,A,b)
!!#### PURPOSE
...
!!--begin--
!get row
row    =   ROW_QDF03_Cell(n)
!get columns
cols   =  COLS_QDF03_Cell(n)
!get coefficients
coeff  = COEFF_QDF03_Cell(n)
...
SUBROUTINE RESIDUAL_QDF03_IC(res,relres,x,label)
!!#### PURPOSE
...
!! get row
row    =   ROW_QDF03_Interface(spec2,spec5)
!! get cols
cols   =  COLS_QDF03_Interface(spec2,spec5)
!! get coeff
coeff  = COEFF_QDF03_Interface(spec2,spec5)
!get right side
rhs    =   RHS_QDF03_Interface(spec2,spec5)
...
SUBROUTINE UPDATE_QDF03_InterfaceCondition(A,b)
!!#### PURPOSE
...
IF( InterfaceType==QDF_SC_plus_WF .OR. InterfaceType==QDF_SC_plus_WEF )THEN
 !! get row
 row    =   ROW_QDF03_Interface(QDF_Flux,QDF_Weak)
 !! get cols
 cols   =  COLS_QDF03_Interface(QDF_Flux,QDF_Weak)
 !! get coeff
 coeff  = COEFF_QDF03_Interface(QDF_Flux,QDF_Weak)
 !! get right side
 b(row) = RHS_QDF03_Interface(QDF_Flux,QDF_Weak)

!weak current
ELSE
 !! get row
 row    =   ROW_QDF03_Interface(QDF_Current,QDF_Weak)
 !! get cols
 cols   =  COLS_QDF03_Interface(QDF_Current,QDF_Weak)
 !! get coeff
 coeff  = COEFF_QDF03_Interface(QDF_Current,QDF_Weak)
 !! get right side
 b(row) = RHS_QDF03_Interface(QDF_Current,QDF_Weak)
END IF
...
DO js0=1,SIZE(Interfaces(jn0)%subs)
 ...
 IF( InterfaceType==QDF_SC_plus_WF .OR. InterfaceType==QDF_SC_plus_WEF )THEN
  !! get row
  row    =   ROW_QDF03_Interface(QDF_Current,QDF_Strong)
  !! get cols
  cols   =  COLS_QDF03_Interface(QDF_Current,QDF_Strong)
  !! get coeff
  coeff  = COEFF_QDF03_Interface(QDF_Current,QDF_Strong)
  !! get right side
  b(row) = RHS_QDF03_Interface(QDF_Current,QDF_Strong)

 !strong flux
 ELSE
  !! get row
  row    =   ROW_QDF03_Interface(QDF_Flux,QDF_Strong)
  !! get cols
  cols   =  COLS_QDF03_Interface(QDF_Flux,QDF_Strong)
  !! get coeff
  coeff  = COEFF_QDF03_Interface(QDF_Flux,QDF_Strong)
  !! get right side
  b(row) = RHS_QDF03_Interface(QDF_Flux,QDF_Strong)
 END IF
...
SUBROUTINE UPDATE_QDF03_BoundaryCondition(A,b)
!!#### PURPOSE
...
!!--begin--
!! get row
row    =   ROW_QDF03_BoundaryFace(BC0,LOBC0)
!! get cols
cols   =  COLS_QDF03_BoundaryFace(BC0,LOBC0)
!! get coeff
coeff  = COEFF_QDF03_BoundaryFace(BC0,LOBC0)
...
CALL SetRow (A, row, cols(1:N) , coeff(1:N) )
!get right side
b(row) = RHS_QDF03_BoundaryFace(BC0,LOBC0)
...
SUBROUTINE RESIDUAL_QDF03_INTERIOR(n,res,relres,x,label)
!!#### PURPOSE
...
SUBROUTINE RESIDUAL_QDF03_BC(res,relres,x,label)
!!#### PURPOSE
...
coeff  = COEFF_QDF03_BoundaryFace(BC0,LOBC0)
!get right side
rhs = RHS_QDF03_BoundaryFace(BC0,LOBC0)
...
FUNCTION RHS_QDF03_Cell(spec2) RESULT(RHS)
!!#### PURPOSE
...
FUNCTION RHS_QDF03_BoundaryFace(BC0,LOBC0) RESULT(RHS)
!!#### REQUIRED INPUT
...
FUNCTION COEFF_QDF03_BoundaryFace(BC0,LOBC0) RESULT(Coeff)
!!#### PURPOSE
...
FUNCTION COLS_QDF03_BoundaryFace(BC0,LOBC0) RESULT(COLS)
!!#### REQUIRED INPUT
INTEGER,INTENT(IN) :: BC0,LOBC0
...
SELECT CASE( LOBC0 )
 ...
 CASE(QDF_fixedJ)      ; cols(1) = COL_QDF03_Face(QDF_Current)
 CASE(QDF_fixedPhi)    ; cols(1) = COL_QDF03_Face(QDF_Flux)
 CASE(QDF_functionJ  ) ; cols(1) = COL_QDF03_Face(QDF_Current)
 CASE(QDF_functionPhi) ; cols(1) = COL_QDF03_Face(QDF_Flux)

 CASE(QDF_functionMixed) ; cols(1)=COL_QDF03_Face(QDF_Current)
                           cols(2)=COL_QDF03_Face(QDF_Flux)

 !normal handling
 CASE DEFAULT

   IF( IsReflective(BC0) )THEN
    cols(1) = COL_QDF03_Face(QDF_Current)
   ELSE
    cols(1) = COL_QDF03_Face(QDF_Current)
    cols(2) = COL_QDF03_Face(QDF_Flux   )
   END IF
...
FUNCTION COLS_QDF03_Cell( spec3 ) RESULT(cols)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_ = "COLS_QDF03_Cell"
...
SELECT CASE( spec3 )
  CASE(QDF_Moment0Center)
    cols(1) = COL_QDF03_Cell(QDF_Current,QDF_Bottom)
    cols(2) = COL_QDF03_Cell(QDF_Current,QDF_Right )
    cols(3) = COL_QDF03_Cell(QDF_Current,QDF_Top   )
    cols(4) = COL_QDF03_Cell(QDF_Current,QDF_Left  )
    cols(5) = COL_QDF03_Cell(QDF_Flux   ,QDF_Center)

  CASE(QDF_Moment1Bottom)
    cols(1) = COL_QDF03_Cell(QDF_Flux   ,QDF_Bottom)
    cols(2) = COL_QDF03_Cell(QDF_Flux   ,QDF_Right )
    cols(3) = COL_QDF03_Cell(QDF_Flux   ,QDF_Center)
    cols(4) = COL_QDF03_Cell(QDF_Flux   ,QDF_Left  )
    cols(5) = COL_QDF03_Cell(QDF_Current,QDF_Bottom)

  CASE(QDF_Moment1Right )
    cols(1) = COL_QDF03_Cell(QDF_Flux   ,QDF_Bottom)
    cols(2) = COL_QDF03_Cell(QDF_Flux   ,QDF_Right )
    cols(3) = COL_QDF03_Cell(QDF_Flux   ,QDF_Top   )
    cols(4) = COL_QDF03_Cell(QDF_Flux   ,QDF_Center)
    cols(5) = COL_QDF03_Cell(QDF_Current,QDF_Right )

  CASE(QDF_Moment1Top   )
    cols(1) = COL_QDF03_Cell(QDF_Flux   ,QDF_Center)
    cols(2) = COL_QDF03_Cell(QDF_Flux   ,QDF_Right )
    cols(3) = COL_QDF03_Cell(QDF_Flux   ,QDF_Top   )
    cols(4) = COL_QDF03_Cell(QDF_Flux   ,QDF_Left  )
    cols(5) = COL_QDF03_Cell(QDF_Current,QDF_Top   )

  CASE(QDF_Moment1Left  )
    cols(1) = COL_QDF03_Cell(QDF_Flux   ,QDF_Bottom)
    cols(2) = COL_QDF03_Cell(QDF_Flux   ,QDF_Center)
    cols(3) = COL_QDF03_Cell(QDF_Flux   ,QDF_Top   )
    cols(4) = COL_QDF03_Cell(QDF_Flux   ,QDF_Left  )
    cols(5) = COL_QDF03_Cell(QDF_Current,QDF_Left  )
...
FUNCTION COEFF_QDF03_Cell( spec3 ) RESULT(coeff)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_ = "COEFF_QDF03_Cell"
...
SELECT CASE( spec3 )
  ...
  CASE(QDF_Moment1Bottom)
    extra    =   0.5_KIND_QDF*((0.5_KIND_QDF*lR(2)*( nB(1)*ERxx0 + nB(2)*ERxy0 ) - &
                                0.5_KIND_QDF*lR(1)*( nB(1)*ERxy0 + nB(2)*ERyy0 )   )) + &
                 0.5_KIND_QDF*((0.5_KIND_QDF*lL(2)*( nB(1)*ELxx0 + nB(2)*ELxy0 ) - &
                                0.5_KIND_QDF*lL(1)*( nB(1)*ELxy0 + nB(2)*ELyy0 )   ))

        coeff(1) =  (1.0_KIND_QDF*lB(2)*( nB(1)*EBxx0 + nB(2)*EBxy0 ) - &
                 1.0_KIND_QDF*lB(1)*( nB(1)*EBxy0 + nB(2)*EByy0 )   ) + extra

        coeff(2) =   0._KIND_QDF

        coeff(3) =  (1.0_KIND_QDF*lh(2)*( nB(1)*ECxx0 + nB(2)*ECxy0 ) - &
                 1.0_KIND_QDF*lh(1)*( nB(1)*ECxy0 + nB(2)*ECyy0 )   ) + extra
  ...
  CASE(QDF_Moment1Right )
        extra    =   0.5_KIND_QDF*((0.5_KIND_QDF*lB(2)*( nR(1)*EBxx0 + nR(2)*EBxy0 ) - &
                                0.5_KIND_QDF*lB(1)*( nR(1)*EBxy0 + nR(2)*EByy0 )   )) + &
                                 0.5_KIND_QDF*((0.5_KIND_QDF*lT(2)*( nR(1)*ETxx0 + nR(2)*ETxy0 ) - &
                                0.5_KIND_QDF*lT(1)*( nR(1)*ETxy0 + nR(2)*ETyy0 )   ))

        coeff(1) =   0._KIND_QDF

        coeff(2) =  (1.0_KIND_QDF*lR(2)*( nR(1)*ERxx0 + nR(2)*ERxy0 ) - &
                 1.0_KIND_QDF*lR(1)*( nR(1)*ERxy0 + nR(2)*ERyy0 )   ) + extra

        coeff(3) =   0._KIND_QDF

        coeff(4) = -(1.0_KIND_QDF*lv(2)*( nR(1)*ECxx0 + nR(2)*ECxy0 ) - &
                 1.0_KIND_QDF*lv(1)*( nR(1)*ECxy0 + nR(2)*ECyy0 )   ) + extra

        coeff(5) = mact0*VR


  CASE(QDF_Moment1Top   )
    extra    =   0.5_KIND_QDF*((0.5_KIND_QDF*lR(2)*( nT(1)*ERxx0 + nT(2)*ERxy0 ) - &
                                    0.5_KIND_QDF*lR(1)*( nT(1)*ERxy0 + nT(2)*ERyy0 )   )) + &
                 0.5_KIND_QDF*((0.5_KIND_QDF*lL(2)*( nT(1)*ELxx0 + nT(2)*ELxy0 ) - &
                                0.5_KIND_QDF*lL(1)*( nT(1)*ELxy0 + nT(2)*ELyy0 )   ))

        coeff(1) = -(1.0_KIND_QDF*lh(2)*( nT(1)*ECxx0 + nT(2)*ECxy0 ) - &
                 1.0_KIND_QDF*lh(1)*( nT(1)*ECxy0 + nT(2)*ECyy0 )   ) + extra

        coeff(2) =   0._KIND_QDF

    coeff(3) =  (1.0_KIND_QDF*lT(2)*( nT(1)*ETxx0 + nT(2)*ETxy0 ) - &
                 1.0_KIND_QDF*lT(1)*( nT(1)*ETxy0 + nT(2)*ETyy0 )   ) + extra
  ...
  CASE(QDF_Moment1Left  )
    extra    =  0.5_KIND_QDF*((0.5_KIND_QDF*lB(2)*( nL(1)*EBxx0 + nL(2)*EBxy0 ) - &
                               0.5_KIND_QDF*lB(1)*( nL(1)*EBxy0 + nL(2)*EByy0 )   ))  + &
                                0.5_KIND_QDF*((0.5_KIND_QDF*lT(2)*( nL(1)*ETxx0 + nL(2)*ETxy0 ) - &
                               0.5_KIND_QDF*lT(1)*( nL(1)*ETxy0 + nL(2)*ETyy0 )   ))

        coeff(1) =   0._KIND_QDF

        coeff(2) =  (1.0_KIND_QDF*lv(2)*( nL(1)*ECxx0 + nL(2)*ECxy0 ) - &
                 1.0_KIND_QDF*lv(1)*( nL(1)*ECxy0 + nL(2)*ECyy0 )   ) + extra

        coeff(3) =   0._KIND_QDF

        coeff(4) =  (1.0_KIND_QDF*lL(2)*( nL(1)*ELxx0 + nL(2)*ELxy0 ) - &
                 1.0_KIND_QDF*lL(1)*( nL(1)*ELxy0 + nL(2)*ELyy0 )   ) + extra
...
FUNCTION COL_QDF03_Cell( spec1 , spec2 ) RESULT(col)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_ = "COL_QDF03_Cell"
...
FUNCTION COL_QDF03_Face( spec1 ) RESULT(col)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_ = "COL_QDF03_Face"
...
FUNCTION ROW_QDF03_Cell( spec3 ) RESULT(row)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_ = "ROW_QDF03_Cell"
...
ELSE
 ...
 SELECT CASE( spec3 )
   CASE(QDF_Moment0Center) ;         ; row = COL_QDF03_Cell(QDF_Flux,QDF_Center) ! i0
   CASE(QDF_Moment1Bottom) ; j0 = jB ; row = COL_QDF03_Face(MERGE(QDF_Current,QDF_Flux,IsMaster(j0)))
   CASE(QDF_Moment1Right ) ; j0 = jR ; row = COL_QDF03_Face(MERGE(QDF_Current,QDF_Flux,IsMaster(j0)))
   CASE(QDF_Moment1Top   ) ; j0 = jT ; row = COL_QDF03_Face(MERGE(QDF_Current,QDF_Flux,IsMaster(j0)))
   CASE(QDF_Moment1Left  ) ; j0 = jL ; row = COL_QDF03_Face(MERGE(QDF_Current,QDF_Flux,IsMaster(j0)))
   CASE DEFAULT
     VS = MODPROC(mod_,proc_)//"this equation specification does not exist."
         CALL Stop(s=STR(VS))
         VS = ""
...
FUNCTION ROW_QDF03_BoundaryFace(BC0,LOBC0) RESULT(ROW)
...
ELSE
 ...
 row = COL_QDF03_Face(QDF_Current)
 row = row + (g0-1)*offsets(3)
...
FUNCTION ROW_QDF03_InterFace(spec2,spec5) RESULT(ROW)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_="ROW_QDF03_InterFace"
...
FUNCTION COLS_QDF03_InterFace(spec2,spec5) RESULT(COLS)
!!#### REQUIRED INPUT
INTEGER,INTENT(IN) :: spec2,spec5
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_="COLS_QDF03_InterFace"
...
SELECT CASE(spec5)
  ...
  CASE(QDF_Weak)
    j0      = Interfaces(jn0)%master
    cols(1) = COL_QDF03_Face(spec2)
    DO js0 = 1,SIZE(Interfaces(jn0)%subs)
     j0          = Interfaces(jn0)%subs(js0)
     cols(1+js0) = COL_QDF03_Face(spec2)
    END DO
...
FUNCTION COEFF_QDF03_Interface(spec2,spec5) RESULT(Coeff)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_="COEFF_QDF03_InterFace"
...
FUNCTION RHS_QDF03_Interface(spec2,spec5) RESULT(rhs)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_="rhs_QDF03_InterFace"
