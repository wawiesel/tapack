      ...
      MODULE USR_CShep2
      !!#PURPOSE
      !! Constructs an interpolating spline for two dimensions
      !! via a modified Cubic Shepard method (see below for details).

      !!#ARCHITECT
      !!   William A. Wieselquist | william.wieselquist AT gmail.com
      !!
      !! Simple modifications to behave in a module.
      !!  @ commented out EXTERNAL and flagged with [WAW]
      !!  @ replaced procedure END with END FUNCTION and END SUBROUTINE
      !!  @ added default private and public only for CSHEP2 and CVAL2

      !!#DEFAULT ACCESS
      PRIVATE

      !!#PUBLIC ACCESS LIST
      PUBLIC :: CSHEP2,CS2VAL

      CONTAINS

C--**--CH3808--792--C:Fix--13:6:2000
C--**--CH3805--790--P:DP--13:6:2000
      SUBROUTINE CSHEP2(N,X,Y,F,NC,NW,NR,LCELL,LNEXT,XMIN,YMIN,DX,DY,
     +                  RMAX,RW,A,IER)
C
C***********************************************************
C
...
C     ..
C     .. Scalar Arguments ..
      DOUBLE PRECISION DX,DY,RMAX,XMIN,YMIN
      INTEGER IER,N,NC,NR,NW
C     ..
C     .. Array Arguments ..
      DOUBLE PRECISION A(9,N),F(N),RW(N),X(N),Y(N)
      INTEGER LCELL(NR,NR),LNEXT(N)
C     ..
C     .. Local Scalars ..
      DOUBLE PRECISION C,DDX,DDY,DMIN,DTOL,FK,RC,RS,RSMX,RSOLD,RTOL,RWS,
     +                 S,SF,SFC,SFS,STF,SUM,T,XK,XMN,YK,YMN
      INTEGER I,IERR,IP1,IRM1,IROW,J,JP1,K,LMAX,LNP,NCWMAX,NEQ,NN,NNC,
     +        NNR,NNW,NP
C     ..
C     .. Local Arrays ..
...
c WAW      EXTERNAL GETNP2,GIVENS,ROTATE,SETUP2,STORE2
C     ..
...
C
      ...
      NCWMAX = MAX(NNC,NNW)
      LMAX = MIN(LMX,NN-1)
      IF (NNC.LT.9 .OR. NNW.LT.1 .OR. NCWMAX.GT.LMAX .OR.
     +    NNR.LT.1) GO TO 170
C
...
C
   10     SUM = SUM + RS
          IF (LNP.EQ.LMAX) GO TO 20
          LNP = LNP + 1
          RSOLD = RS
          CALL GETNP2(XK,YK,X,Y,NNR,LCELL,LNEXT,XMN,YMN,DDX,DDY,NP,RS)
          IF (RS.EQ.0.) GO TO 180
          NPTS(LNP) = NP
          IF ((RS-RSOLD)/RS.LT.RTOL) GO TO 10
...
C
   ...
   40     I = I + 1
          NP = NPTS(I)
          IROW = MIN(I,10)
          CALL SETUP2(XK,YK,FK,X(NP),Y(NP),F(NP),SF,SFS,SFC,RC,
     +                B(1,IROW))
          IF (I.EQ.1) GO TO 40
          IRM1 = IROW - 1
          DO 50 J = 1,IRM1
...
C
     ...
     +           ABS(B(9,9)))
          IF (DMIN*RC.GE.DTOL) GO TO 110
          IF (NEQ.EQ.LMAX) GO TO 70
C
...
C
              ...
              IF ((RS-RSOLD)/RS.LT.RTOL) GO TO 60
              RC = SQRT(RS)
              GO TO 40
...
C
          DMIN = MIN(ABS(B(5,5)),ABS(B(6,6)),ABS(B(7,7)),ABS(B(8,8)),
     +           ABS(B(9,9)))
          IF (DMIN*RC.LT.DTOL) GO TO 190
C
...
C
  190 XMIN = XMN
      ...
      DOUBLE PRECISION FUNCTION CS2VAL(PX,PY,N,X,Y,F,NR,LCELL,LNEXT,
...
C
C     .. Scalar Arguments ..
      DOUBLE PRECISION DX,DY,PX,PY,RMAX,XMIN,YMIN
      INTEGER N,NR
C     ..
C     .. Array Arguments ..
      DOUBLE PRECISION A(9,N),F(N),RW(N),X(N),Y(N)
...
C     ..
      XP = PX
      YP = PY
      IF (N.LT.10 .OR. NR.LT.1 .OR. DX.LE.0. .OR. DY.LE.0. .OR.
     +    RMAX.LT.0.) RETURN
C
...
C   of radius RMAX.
C
      IF (IMIN.GT.IMAX .OR. JMIN.GT.JMAX) GO TO 60
C
...
C
   10         DELX = XP - X(K)
              ...
              W = (1.0/D-1.0/R)**3
              SW = SW + W
              SWC = SWC + W* (((A(1,K)*DELX+A(2,K)*DELY+A(5,
     +              K))*DELX+ (A(3,K)*DELY+A(6,K))*DELY+A(8,K))*DELX+
...
C
   60 CS2VAL = 0.
      ...
      SUBROUTINE CS2GRD(PX,PY,N,X,Y,F,NR,LCELL,LNEXT,XMIN,YMIN,DX,DY,
     +                  RMAX,RW,A,C,CX,CY,IER)
C
C***********************************************************
C
C                                               From CSHEP2D
...
C
C     .. Scalar Arguments ..
      DOUBLE PRECISION C,CX,CY,DX,DY,PX,PY,RMAX,XMIN,YMIN
      INTEGER IER,N,NR
C     ..
C     .. Array Arguments ..
      DOUBLE PRECISION A(9,N),F(N),RW(N),X(N),Y(N)
...
C     ..
C     .. Local Scalars ..
      DOUBLE PRECISION CK,CKX,CKY,D,DELX,DELY,R,SW,SWC,SWCX,SWCY,SWS,
     +                 SWX,SWY,T,W,WX,WY,XP,YP
      INTEGER I,IMAX,IMIN,J,JMAX,JMIN,K,KP
C     ..
C     .. Intrinsic Functions ..
      INTRINSIC INT,SQRT
...
C   of radius RMAX.
C
      IF (IMIN.GT.IMAX .OR. JMIN.GT.JMAX) GO TO 70
C
...
C
   10         DELX = XP - X(K)
              ...
              CKY = (3.0*A(4,K)*DELY+A(3,K)*DELX+2.0*A(7,K))*DELY +
     +              T*DELX + A(9,K)
              T = T*DELY + A(8,K)
...
C
   70 C = 0.
      ...
      SUBROUTINE CS2HES(PX,PY,N,X,Y,F,NR,LCELL,LNEXT,XMIN,YMIN,DX,DY,
     +                  RMAX,RW,A,C,CX,CY,CXX,CXY,CYY,IER)
C
C***********************************************************
C
C                                               From CSHEP2D
...
C
C     .. Scalar Arguments ..
      DOUBLE PRECISION C,CX,CXX,CXY,CY,CYY,DX,DY,PX,PY,RMAX,XMIN,YMIN
      INTEGER IER,N,NR
C     ..
C     .. Array Arguments ..
      DOUBLE PRECISION A(9,N),F(N),RW(N),X(N),Y(N)
...
C     ..
C     .. Local Scalars ..
      DOUBLE PRECISION CK,CKX,CKXX,CKXY,CKY,CKYY,D,DELX,DELY,DXSQ,DYSQ,
     +                 R,SW,SWC,SWCX,SWCXX,SWCXY,SWCY,SWCYY,SWS,SWX,
     +                 SWXX,SWXY,SWY,SWYY,T1,T2,T3,T4,W,WX,WXX,WXY,WY,
     +                 WYY,XP,YP
      INTEGER I,IMAX,IMIN,J,JMAX,JMIN,K,KP
C     ..
C     .. Intrinsic Functions ..
...
C
C     .. Scalar Arguments ..
      DOUBLE PRECISION DSQ,DX,DY,PX,PY,XMIN,YMIN
      INTEGER NP,NR
C     ..
C     .. Array Arguments ..
      DOUBLE PRECISION X(*),Y(*)
      INTEGER LCELL(NR,NR),LNEXT(*)
C     ..
C     .. Local Scalars ..
...
C
      IF (NR.LT.1 .OR. DX.LE.0. .OR. DY.LE.0.) GO TO 90
C
...
C
   10 DO 60 J = J1,J2
          IF (J.GT.JMAX) GO TO 70
          IF (J.LT.JMIN) GO TO 60
          DO 50 I = I1,I2
              IF (I.GT.IMAX) GO TO 60
              IF (I.LT.IMIN) GO TO 50
              IF (J.NE.J1 .AND. J.NE.J2 .AND. I.NE.I1 .AND.
     +            I.NE.I2) GO TO 50
C
...
C
   70 IF (I1.LE.IMIN .AND. I2.GE.IMAX .AND. J1.LE.JMIN .AND.
     +    J2.GE.JMAX) GO TO 80
      I1 = I1 - 1
      I2 = I2 + 1
      J1 = J1 - 1
...
C
C     .. Scalar Arguments ..
      DOUBLE PRECISION R,S1,S2,S3,XI,XK,YI,YK,ZI,ZK
C     ..
C     .. Array Arguments ..
...
C     ..
      NN = N
      NNR = NR
      IF (NN.LT.2 .OR. NNR.LT.1) GO TO 50
C
...
C
      ...
      DX = DELX
      DY = DELY
      IF (DELX.EQ.0. .OR. DELY.EQ.0.) GO TO 60
C
C Initialize LCELL.
C
      DO 30 J = 1,NNR
