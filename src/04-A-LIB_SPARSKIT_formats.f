cc# MODULE  <<LIB_SPARSKIT_formats>>
        MODULE LIB_SPARSKIT_formats

cc## PURPOSE
cc Provide format conversions for various sparse matrix
cc representations.


cc## MODIFICATIONS
cc {May2006}
cc   [waw] | William A. Wieselquist | william.wieselquist AT gmail.com
cc   * conversion to module
...
c
c[waw] - added integer work array
...
c[waw]   call csort (nrow, al, jal, ial, au, .true.)
c[waw] - changed <au> to <iwork>
         allocate( iwork(kl) )
...
c-----sort matrix by column indices
      ...
      if (.not. sorted) then
c[waw] - replaced <b> with <iwork>
c[waw]        call csort (n, a, ja, ia, b, .true.)
...
c-----------------------------------------------------------------------
c------------------------end-of-csorted---------------------------------
      subroutine csrcsc666ao (n,job,ipos,a,ja,ia,ao,jao,iao)
      integer ia(n+1),iao(n+1),ja(*),jao(*)
      integer a(*)
        real*8  ao(*)
c-----------------------------------------------------------------------
c Compressed Sparse Row     to      Compressed Sparse Column
c-----------------------------------------------------------------------
c                 william.wieselquist@gmail.com
c-----------------------------------------------------------------------
      call csrcsc666ao2 (n,n,job,ipos,a,ja,ia,ao,jao,iao)
      end subroutine
c-----------------------------------------------------------------------
      subroutine csrcsc666ao2 (n,n2,job,ipos,a,ja,ia,ao,jao,iao)
      integer ia(n+1),iao(n2+1),ja(*),jao(*)
      integer a(*)
        real*8 ao(*)
c-----------------------------------------------------------------------
c Compressed Sparse Row     to      Compressed Sparse Column
c-----------------------------------------------------------------------
c This version handles the case where a is an integer array
c
c          william.wieselquist@gmail.com
c-----------------------------------------------------------------------
c----------------- compute lengths of rows of transp(A) ----------------
...
c-------------------------- reshift iao and leave ----------------------
 ...
 7    continue
      iao(1) = ipos
c--------------- end of csrcsc666ao-------------------------------------
c-----------------------------------------------------------------------
      end subroutine

      subroutine csrcsc666a (n,job,ipos,a,ja,ia,ao,jao,iao)
      integer ia(n+1),iao(n+1),ja(*),jao(*)
      real*8  a(*)
        integer ao(*)
c-----------------------------------------------------------------------
c Compressed Sparse Row     to      Compressed Sparse Column
c-----------------------------------------------------------------------
c This version handles the case where ao is an integer array
c
c          william.wieselquist@gmail.com
c-----------------------------------------------------------------------
      call csrcsc666a2 (n,n,job,ipos,a,ja,ia,ao,jao,iao)
      end subroutine
c-----------------------------------------------------------------------
      subroutine csrcsc666a2 (n,n2,job,ipos,a,ja,ia,ao,jao,iao)
      integer ia(n+1),iao(n2+1),ja(*),jao(*)
      real*8  a(*)
        integer ao(*)
c-----------------------------------------------------------------------
