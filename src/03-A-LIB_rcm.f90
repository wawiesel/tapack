...
!!### PURPOSE
!! RCM is a FORTRAN90 program which computes the reverse
!! Cuthill McKee ("RCM") ordering of the nodes of a graph.


!!### DETAILS
!! The RCM ordering is frequently used when a matrix is to
!! be generated whose rows and columns are numbered
!! according to the numbering of the nodes. By an
!! appropriate renumbering of the nodes, it is often possible
!! to produce a matrix with a much smaller bandwidth.
!!
!! The bandwidth of a matrix is computed as the maximum
!! bandwidth of each row of the matrix. The bandwidth of a
!! row of the matrix is essentially the number of matrix
!! entries between the first and last nonzero entries in
!! the row, with the proviso that the diagonal entry is
!! always treated as though it were nonzero.
!!
!! This library includes a few routines to handle
!! the common case where the connectivity can be
!! described in terms of a triangulation of the nodes,
!! that is, a grouping of the nodes into sets
!! of 3-node or 6-node triangles. The natural description
!! of a triangulation is simply a listing of the nodes
!! that make up each triangle. The library includes
!! routines for determining the adjacency structure associated
!! with a triangulation, and the test problems include
!! examples of how the nodes in a triangulation can be
!! relabeled with the RCM permutation.
...
!   3. H L Crane, Norman Gibbs, William Poole, Paul Stockmeyer,
!      Algorithm 508: Matrix Bandwidth and Profile Reduction,
!      ACM Transactions on Mathematical Software,
!      Volume 2, Issue 4, pages 375-377, 1976.
!   4. Norman Gibbs,
...
!  lvlend points to the end subroutine of this level.
!
  do
...
!*******************************************************************************
!
!! IROW_SORT_A ascend subroutineing sorts the rows of an integer array.
!
!  Discussion:
!
...
!    On output, the rows of A have been sorted in ascend subroutineing
!    lexicographic order.
!
  implicit none
...
!  the end subroutine of the current level respectively.
!
...
!
!  In the unlikely case that ROOT is one end subroutinepoint of a line graph,
!  we can exit now.
!
...
!*******************************************************************************
!
!! SORT_HEAP_EXTERNAL externally sorts a list of items into ascend subroutineing order.
!
!  Discussion:
!
...
!
  ...
  write ( string, '(a,1x,i2,1x,i4,2x,i2,a1,i6.3,a1,i6.3,a1,i6.3,1x,a)' ) &
...
subroutine triang_order3_example_size ( node_num, tri_num )
...
subroutine triang_order3_neigh_triangles ( tri_num, tri_vert, &
  tri_nabe )
...
!
!  Step 2. Perform an ascend subroutineing dictionary sort on the neighbor relations.
!  We only intend subroutine to sort on columns 1 and 2; the routine we call here
...
subroutine triang_order6_example_size ( node_num, tri_num )
...
subroutine triang_order6_neigh_triangles ( tri_num, tri_vert, &
  tri_nabe )
