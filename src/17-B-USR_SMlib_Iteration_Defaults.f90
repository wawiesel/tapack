MODULE USR_SMlib_Iteration_Defaults
  ! iteration defaults modified by
  ! william.wieselquist@gmail.com to change
  !   NONE->SMLIB_NONE,
  !   SOME->SMLIB_SOME,
  !   ALL->SMLIB_ALL
  !
  ...
  !
  !!   Parameters:
  !!       SMLIB_NONE, SMLIB_SOME, SMLIB_ALL -
  !!         Defining the amount of information the iterative
  ...
  !!  Get the library working real precision.
  !!  On some Crays, it is possible to count
  !!  float point operations.

  use USR_SMlib_Precision, ONLY: prec !!((16-B-USR_SMlib_Precision.f90))
  ...
  PUBLIC :: ConvHist, Write_ConvHist, Initiate_ConvHist, SetIterationHistory, Nullify_ConvHist, &
       & SMLIB_NONE, SMLIB_SOME, SMLIB_ALL, F1, F2, F3, &
...
![waw]    INTERFACE CPU_TIME
![waw]       SUBROUTINE CPU_TIME (TIME)
![waw]         REAL :: TIME
![waw]       END SUBROUTINE CPU_TIME
![waw]    END INTERFACE
...
END MODULE USR_SMlib_Iteration_Defaults
