!!# USER MODULE: <USR_Quasidiffusion05>
MODULE USR_Quasidiffusion05

!!## PURPOSE
!! The user-module for the fourth (Morel) variant
!! of the quasidiffusion routines with extra capability
!! to perform calculations with something other than
!! $\sigma_a$ in the QD method.
...
!!## ACCESS
PUBLIC :: SETUP_QDF05,UPDATE_QDF05,RESIDUAL_QDF05,RECALL_QDF05,XINIT_QDF05
PUBLIC :: PUT_QDF05,COL_QDF05_Cell,COL_QDF05_Face
!!## IDENTIFICATION
CHARACTER(*),PARAMETER :: mod_="USR_Quasidiffusion05"
...
INTEGER       :: jB,jR,jT,jL
REAL(KIND_MAC) :: macs0,COEFF_macs0
REAL(KIND_MAC) :: mact0,COEFF_mact0
INTEGER               :: BC0,LOBC0
INTEGER          :: save_row = 0
...
SUBROUTINE SETUP_QDF05( Ng , Mesh , A , b , x , Jbdry , Jdomn , FdBk , &
 Return_Interfaces )
!!#### PURPOSE
...
SUBROUTINE SETUP_QDF05_INTERIOR(g,i,Mesh,mact,macs,l_,ExtSource,&
  ECxx , ECyy , ECxy , Exx , Eyy , Exy , COEFF_macs, COEFF_mact, Noisy,Unit,Interactive,ExtSourceF1)
...
!! Get cross sections.
mact0 = mact(g0,l_(i0))
IF( .NOT.ASSOCIATED(COEFF_mact) )THEN
    COEFF_mact0=1.d0
ELSE
    COEFF_mact0 = COEFF_mact(g0,i0)
END IF

macs0 = macs(g0,l_(i0))
IF( .NOT.ASSOCIATED(COEFF_macs) )THEN
    COEFF_macs0=1.d0
ELSE
    COEFF_macs0 = COEFF_macs(g0,i0)
END IF
...
IF( Noisy_ )THEN
 ...
 WRITE(Unit_,101)"     * macro scattering coefficient=",COEFF_macs0
 WRITE(Unit_,101)"     * macro transport  coefficient=",COEFF_mact0
END IF
...
SUBROUTINE PUT_QDF05(g,i,jb,jd,j)
!!#### OPTIONAL INPUT
...
SUBROUTINE RESIDUAL_QDF05( res , relres , x , iter , Mesh , &
 mact , macs , l_ , ExtSource , ECxx , ECyy , ECxy , Exx , &
 Eyy , Exy , Jbdry , Jdomn , C , ScalarFluxIN , CurrentIN , &
 BC , LOBC , FixBdryVals , COEFF_macs, COEFF_mact, FdBk , Noisy,Unit,Interactive,Unit_resid,&
 ExtSourceF1)
...
DO g=1,Ng0
 ...
 DO i=1,Ni0
  ...
  CALL SETUP_QDF05_INTERIOR(g,i,Mesh,mact,macs,l_,ExtSource,&
    ECxx , ECyy , ECxy , Exx , Eyy , Exy , COEFF_macs, COEFF_mact, &
    Noisy=.FALSE.,Unit=0,Interactive=.FALSE.,ExtSourceF1=ExtSourceF1)

  !1. for each of 5 equations
  DO n=1,5
   IF( Unit_resid_/=0 )THEN
    CALL RESIDUAL_QDF05_INTERIOR(n,res,relres,x,label)
   ELSE
    CALL RESIDUAL_QDF05_INTERIOR(n,res,relres,x)
   END IF
  END DO
 ...
 DO jn=1,Njn0
  ...
  CALL SETUP_QDF05_INTERFACE(g,jn,Mesh,Interfaces, &
    Noisy=.FALSE.,Unit=0,Interactive=.FALSE. )

  IF( Unit_resid_/=0 )THEN
   CALL RESIDUAL_QDF05_IC(res,relres,x,label)
  ELSE
   CALL RESIDUAL_QDF05_IC(res,relres,x)
  END IF
 ...
 DO jb=1,Njb0
  ...
  CALL SETUP_QDF05_BOUNDARY(g,jb,Mesh,Jbdry,Jdomn, C , &
    ScalarFluxIN , CurrentIN , BC , LOBC , FixBdryVals , &
    Noisy=.FALSE.,Unit=0,Interactive=.FALSE.)

  !! 6.b Get the boundary conditions.
  IF( Unit_resid_/=0 )THEN
   CALL RESIDUAL_QDF05_BC(res,relres,x,label)
  ELSE
   CALL RESIDUAL_QDF05_BC(res,relres,x)
  END IF
...
IF( Unit_resid_/=0 )THEN
 WRITE(Unit_resid_,"(a,i6.3,a)")"#QDF05 RESIDUALS (iter=",iter,")#"
...
SUBROUTINE RECALL_QDF05( x , ScalarFluxC , ScalarFluxF , CurrentF )
!!#### PURPOSE
...
DO g=1,Ng0
 ...
 DO i=1,Ni0
  ...
  !cell flux
  n = COL_QDF05_Cell(QDF_Flux,QDF_Center)
  ScalarFluxC(g,i) = x(n)
 END DO
 ...
 DO j=1,Nj0
  ...
  !face flux
  n = COL_QDF05_Face(QDF_Flux)
  ScalarFluxF(g,j) = x(n)

  !face current
  n = COL_QDF05_Face(QDF_Current)
  CurrentF(g,j) = x(n)
 END DO
END DO
...
SUBROUTINE XINIT_QDF05( x , ScalarFluxC , ScalarFluxF , CurrentF )
!!#### PURPOSE
!! Inverse of RECALL_QDF05, where we put the values into x (i.e.
!! initialize x.
...
SUBROUTINE UPDATE_QDF05( A , b , Mesh , mact , macs , l_ , &
 ExtSource , ECxx , ECyy , ECxy , Exx , Eyy , Exy , Jbdry , Jdomn , C , &
 ScalarFluxIN , CurrentIN , BC , LOBC , FixBdryVals , COEFF_macs , COEFF_mact, &
...
INTEGER             ,POINTER    :: LOBC(:)
REAL(KIND_QDF)      ,INTENT(IN) :: FixBdryVals(:)
REAL(KIND_QDF)      ,POINTER    :: COEFF_macs(:,:),COEFF_mact(:,:)
...
CALL UPDATE(fdbk_comment,fdbk,s="[[QDF]] forming the sparse coefficient &
  &matrix <A> and right side <b> for the QDF05 system variant")
...
IF( DEFAULT(.FALSE.,Noisy) )THEN
 WRITE(DEFAULT(window_unit,Unit),"(//,a)")"#QDF05 SETUP"
END IF
...
DO g=1,Ng0
 ...
 DO i=1,Ni0
  ...
  !1. for each of 5 equations
  DO n=1,5
   CALL UPDATE_QDF05_INTERIOR(n,A,b)
  END DO
 ...
 DO jn=1,Njn0
  ...
  CALL UPDATE_QDF05_InterfaceCondition(A,b)
 END DO
 ...
 DO jb=1,Njb0
  ...
  !! 6.b Get the boundary conditions.
  CALL UPDATE_QDF05_BoundaryCondition(A,b)
 END DO
END DO
!(/hack/)
!write(*,*)row_used
...
CALL UPDATE(fdbk_comment,fdbk,s="[[QDF]] equation/unknown count for QDF05 system: generated <"&
...
SUBROUTINE SETUP_QDF05_INTERFACE(g,jn,Mesh,Interfaces, &
  Noisy,Unit,Interactive )
!!#### PURPOSE
...
SUBROUTINE SETUP_QDF05_BOUNDARY(g,jb,Mesh,Jbdry,Jdomn, C , &
...
SUBROUTINE UPDATE_QDF05_INTERIOR(n,A,b)
!!#### PURPOSE
...
!!--begin--
!get row
row    =   ROW_QDF05_Cell(n)
!get columns
cols = COLS_QDF05_Cell(n)
!get coefficients
coeff = COEFF_QDF05_Cell(n)
...
SUBROUTINE RESIDUAL_QDF05_IC(res,relres,x,label)
!!#### PURPOSE
...
!! get row
row    =   ROW_QDF05_Interface(spec2,spec5)
!! get cols
cols   =  COLS_QDF05_Interface(spec2,spec5)
!! get coeff
coeff  = COEFF_QDF05_Interface(spec2,spec5)
!get right side
rhs    =   RHS_QDF05_Interface(spec2,spec5)
...
SUBROUTINE UPDATE_QDF05_InterfaceCondition(A,b)
!!#### PURPOSE
...
IF( InterfaceType==QDF_SC_plus_WF .OR. InterfaceType==QDF_SC_plus_WEF )THEN
 !! get row
 row    =   ROW_QDF05_Interface(QDF_Flux,QDF_Weak)
 !! get cols
 cols   =  COLS_QDF05_Interface(QDF_Flux,QDF_Weak)
 !! get coeff
 coeff  = COEFF_QDF05_Interface(QDF_Flux,QDF_Weak)
 !! get right side
 b(row) = RHS_QDF05_Interface(QDF_Flux,QDF_Weak)

!weak current
ELSE
 !! get row
 row    =   ROW_QDF05_Interface(QDF_Current,QDF_Weak)
 !! get cols
 cols   =  COLS_QDF05_Interface(QDF_Current,QDF_Weak)
 !! get coeff
 coeff  = COEFF_QDF05_Interface(QDF_Current,QDF_Weak)
 !! get right side
 b(row) = RHS_QDF05_Interface(QDF_Current,QDF_Weak)
END IF
...
DO js0=1,SIZE(Interfaces(jn0)%subs)
 ...
 IF( InterfaceType==QDF_SC_plus_WF .OR. InterfaceType==QDF_SC_plus_WEF )THEN
  !! get row
  row    =   ROW_QDF05_Interface(QDF_Current,QDF_Strong)
  !! get cols
  cols   =  COLS_QDF05_Interface(QDF_Current,QDF_Strong)
  !! get coeff
  coeff  = COEFF_QDF05_Interface(QDF_Current,QDF_Strong)
  !! get right side
  b(row) = RHS_QDF05_Interface(QDF_Current,QDF_Strong)

 !strong flux
 ELSE
  !! get row
  row    =   ROW_QDF05_Interface(QDF_Flux,QDF_Strong)
  !! get cols
  cols   =  COLS_QDF05_Interface(QDF_Flux,QDF_Strong)
  !! get coeff
  coeff  = COEFF_QDF05_Interface(QDF_Flux,QDF_Strong)
  !! get right side
  b(row) = RHS_QDF05_Interface(QDF_Flux,QDF_Strong)
 END IF
...
SUBROUTINE UPDATE_QDF05_BoundaryCondition(A,b)
!!#### PURPOSE
...
!!--begin--
!! get row
row    =   ROW_QDF05_BoundaryFace(BC0,LOBC0)
!! get cols
cols   =  COLS_QDF05_BoundaryFace(BC0,LOBC0)
!! get coeff
coeff  = COEFF_QDF05_BoundaryFace(BC0,LOBC0)
...
CALL SetRow (A, row, cols(1:N) , coeff(1:N) )
!get right side
b(row) = RHS_QDF05_BoundaryFace(BC0,LOBC0)
...
SUBROUTINE RESIDUAL_QDF05_INTERIOR(n,res,relres,x,label)
!!#### PURPOSE
...
SUBROUTINE RESIDUAL_QDF05_BC(res,relres,x,label)
!!#### PURPOSE
...
coeff  = COEFF_QDF05_BoundaryFace(BC0,LOBC0)
!get right side
rhs = RHS_QDF05_BoundaryFace(BC0,LOBC0)
...
FUNCTION RHS_QDF05_Cell(spec2) RESULT(RHS)
!!#### PURPOSE
...
FUNCTION RHS_QDF05_BoundaryFace(BC0,LOBC0) RESULT(RHS)
!!#### REQUIRED INPUT
...
FUNCTION COEFF_QDF05_BoundaryFace(BC0,LOBC0) RESULT(Coeff)
!!#### PURPOSE
...
FUNCTION COLS_QDF05_BoundaryFace(BC0,LOBC0) RESULT(COLS)
!!#### REQUIRED INPUT
INTEGER,INTENT(IN) :: BC0,LOBC0
...
SELECT CASE( LOBC0 )
 ...
 CASE(QDF_fixedJ)      ; cols(1) = COL_QDF05_Face(QDF_Current)
 CASE(QDF_fixedPhi)    ; cols(1) = COL_QDF05_Face(QDF_Flux)
 CASE(QDF_functionJ  ) ; cols(1) = COL_QDF05_Face(QDF_Current)
 CASE(QDF_functionPhi) ; cols(1) = COL_QDF05_Face(QDF_Flux)

 CASE(QDF_functionMixed) ; cols(1)=COL_QDF05_Face(QDF_Current)
                           cols(2)=COL_QDF05_Face(QDF_Flux)

 !normal handling
 CASE DEFAULT

   IF( IsReflective(BC0) )THEN
    cols(1) = COL_QDF05_Face(QDF_Current)
   ELSE
    cols(1) = COL_QDF05_Face(QDF_Current)
    cols(2) = COL_QDF05_Face(QDF_Flux   )
   END IF
...
FUNCTION COLS_QDF05_Cell( spec3 ) RESULT(cols)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_ = "COLS_QDF05plus_Cell"
...
SELECT CASE( spec3 )
  CASE(QDF_Moment0Center)
    cols(1) = COL_QDF05_Cell(QDF_Current,QDF_Bottom)
    cols(2) = COL_QDF05_Cell(QDF_Current,QDF_Right )
    cols(3) = COL_QDF05_Cell(QDF_Current,QDF_Top   )
    cols(4) = COL_QDF05_Cell(QDF_Current,QDF_Left  )
    cols(5) = COL_QDF05_Cell(QDF_Flux   ,QDF_Center)

  CASE(QDF_Moment1Bottom)
    cols(1) = COL_QDF05_Cell(QDF_Flux   ,QDF_Bottom)
    cols(2) = COL_QDF05_Cell(QDF_Flux   ,QDF_Right )
    cols(3) = COL_QDF05_Cell(QDF_Flux   ,QDF_Center)
    cols(4) = COL_QDF05_Cell(QDF_Flux   ,QDF_Left  )
    cols(5) = COL_QDF05_Cell(QDF_Current,QDF_Bottom)
    cols(6) = COL_QDF05_Cell(QDF_Flux   ,QDF_Top   )

  CASE(QDF_Moment1Right )
    cols(1) = COL_QDF05_Cell(QDF_Flux   ,QDF_Bottom)
    cols(2) = COL_QDF05_Cell(QDF_Flux   ,QDF_Right )
    cols(3) = COL_QDF05_Cell(QDF_Flux   ,QDF_Top   )
    cols(4) = COL_QDF05_Cell(QDF_Flux   ,QDF_Center)
    cols(5) = COL_QDF05_Cell(QDF_Current,QDF_Right )
    cols(6) = COL_QDF05_Cell(QDF_Flux   ,QDF_Left  )

  CASE(QDF_Moment1Top   )
    cols(1) = COL_QDF05_Cell(QDF_Flux   ,QDF_Center)
    cols(2) = COL_QDF05_Cell(QDF_Flux   ,QDF_Right )
    cols(3) = COL_QDF05_Cell(QDF_Flux   ,QDF_Top   )
    cols(4) = COL_QDF05_Cell(QDF_Flux   ,QDF_Left  )
    cols(5) = COL_QDF05_Cell(QDF_Current,QDF_Top   )
    cols(6) = COL_QDF05_Cell(QDF_Flux   ,QDF_Bottom)

  CASE(QDF_Moment1Left  )
    cols(1) = COL_QDF05_Cell(QDF_Flux   ,QDF_Bottom)
    cols(2) = COL_QDF05_Cell(QDF_Flux   ,QDF_Center)
    cols(3) = COL_QDF05_Cell(QDF_Flux   ,QDF_Top   )
    cols(4) = COL_QDF05_Cell(QDF_Flux   ,QDF_Left  )
    cols(5) = COL_QDF05_Cell(QDF_Current,QDF_Left  )
    cols(6) = COL_QDF05_Cell(QDF_Flux   ,QDF_Right )
...
FUNCTION COEFF_QDF05_Cell( spec3 ) RESULT(coeff)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_ = "COEFF_QDF05_Cell"
...
SELECT CASE( spec3 )
  CASE(QDF_Moment0Center)
    ...
    coeff(3) = hT
    coeff(4) = hL
    coeff(5) = (COEFF_mact0*mact0 - COEFF_macs0*macs0)*Vi
...
FUNCTION COL_QDF05_Cell( spec1 , spec2 ) RESULT(col)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_ = "COL_QDF05_Cell"
...
FUNCTION COL_QDF05_Face( spec1 ) RESULT(col)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_ = "COL_QDF05_Face"
...
FUNCTION ROW_QDF05_Cell( spec3 ) RESULT(row)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_ = "ROW_QDF05_Cell"
...
ELSE
 ...
 SELECT CASE( spec3 )
   CASE(QDF_Moment0Center) ;         ; row = COL_QDF05_Cell(QDF_Flux,QDF_Center) ! i0
   CASE(QDF_Moment1Bottom) ; j0 = jB ; row = COL_QDF05_Face(MERGE(QDF_Current,QDF_Flux,IsMaster(j0)))
   CASE(QDF_Moment1Right ) ; j0 = jR ; row = COL_QDF05_Face(MERGE(QDF_Current,QDF_Flux,IsMaster(j0)))
   CASE(QDF_Moment1Top   ) ; j0 = jT ; row = COL_QDF05_Face(MERGE(QDF_Current,QDF_Flux,IsMaster(j0)))
   CASE(QDF_Moment1Left  ) ; j0 = jL ; row = COL_QDF05_Face(MERGE(QDF_Current,QDF_Flux,IsMaster(j0)))
   CASE DEFAULT
     VS = MODPROC(mod_,proc_)//"this equation specification does not exist."
         CALL Stop(s=STR(VS))
         VS = ""
...
FUNCTION ROW_QDF05_BoundaryFace(BC0,LOBC0) RESULT(ROW)
!!#### REQUIRED INPUT
INTEGER,INTENT(IN) :: BC0,LOBC0
...
ELSE
 ...
 row = COL_QDF05_Face(QDF_Current)
 row = row + (g0-1)*offsets(3)
...
FUNCTION ROW_QDF05_InterFace(spec2,spec5) RESULT(ROW)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_="ROW_QDF05_InterFace"
...
FUNCTION COLS_QDF05_InterFace(spec2,spec5) RESULT(COLS)
!!#### REQUIRED INPUT
INTEGER,INTENT(IN) :: spec2,spec5
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_="COLS_QDF05_InterFace"
...
SELECT CASE(spec5)
  ...
  CASE(QDF_Weak)
    j0      = Interfaces(jn0)%master
    cols(1) = COL_QDF05_Face(spec2)
    DO js0 = 1,SIZE(Interfaces(jn0)%subs)
     j0          = Interfaces(jn0)%subs(js0)
     cols(1+js0) = COL_QDF05_Face(spec2)
    END DO
...
FUNCTION COEFF_QDF05_Interface(spec2,spec5) RESULT(Coeff)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_="COEFF_QDF05_InterFace"
...
FUNCTION RHS_QDF05_Interface(spec2,spec5) RESULT(rhs)
!!#### PURPOSE
...
!!#### IDENTIFICATION
CHARACTER(*),PARAMETER :: proc_="rhs_QDF05_InterFace"
