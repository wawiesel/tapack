...
! Interface blocks
  ...
  !![waw] - added to handle varying string pointers
  !!        (return len=0 if ptr is not associated)
  interface len_ptr0
     module procedure len_ptr0_
  end interface len_ptr0
  interface len_ptr1
     module procedure len_ptr1_
  end interface len_ptr1
...
!****
  ...
  pure function char_auto1 (string) result (char_string)

    type(varying_string), intent(in) :: string(:)
    character(LEN=len(string(1)))    :: char_string(1:SIZE(string))
...
! (automatic length)
    ...
    forall(j = 1:SIZE(string))
      forall(i_char = 1:len(string(j)))
        char_string(j)(i_char:i_char) = string(j)%chars(i_char)
      end forall
    end forall
! Finish
...
!****
  ...
  function len_ptr0_ (string) result (length)

    type(varying_string), pointer :: string
    integer                       :: length

! Get the length of a varying string pointer (scalar)
...
! Finish
  ...
  end function len_ptr0_

!****

  function len_ptr1_ (string) result (length)

    type(varying_string), pointer :: string(:)
    integer                       :: length

! Get the length of a varying string pointer (array)

    if(associated(string)) then
       length = maxval(len(string))
    else
       length = 0
    endif
