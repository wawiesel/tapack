...
c     ccnicopy:We copy an integer vector into another.
...
c..........We copy IA and JA on IAT and JAT respectively
           ...
           if(job .eq. 1) call dcopy666(ntb, amat, 1, izs(iamat), 1)
           call dperm666(n, izs(iamat), izs(ijat), izs(iiat), amat,
     *                ja, ia, izs(ilpw), izs(ilpw), job)
           ipos = 1
c..........We sort columns inside JA.
           call csrcsc666a (n, job, ipos, amat      ,
     &        ja       ,        ia, izs(iamat), izs(ijat), izs(iiat))
           call csrcsc666ao(n, job, ipos, izs(iamat),
     &        izs(ijat), izs(iiat), amat      , ja       , ia  )
      endif
