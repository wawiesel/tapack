      module SUB_bicgstab2

	contains

	subroutine bicgstab2 (okprint,l, n, x, rhs, matvec, nonzero, tol,
     &      typestop,mxmv, work, ldw, info, residit, residval)
C
...
C ----------------------------------------------------------
	...
	integer, optional :: residit(mxmv)
	double precision, optional :: residval(mxmv)
C     -----------------------------------------
      integer   lmax
      parameter(lmax=2)
...
C     ================================
         ...
         if(typestop.eq.'rel')then
		  ...
		  ![waw] set the iteration numbers and residuals
		  if( present(residit) .AND. present(residval) )then
		     ppp = ppp+1
		     residit (ppp) = nmv
			 residval(ppp) = rnrm/rnrm0
	      end if ![waw]
		  ...
		  ![waw] output max residuals
		  if( present(residit) .AND. present(residval) )then
		     ppp = ppp+1
		     residit (ppp) = nmv
			 residval(ppp) = rnrmMax
	      end if ![waw]
         end if
